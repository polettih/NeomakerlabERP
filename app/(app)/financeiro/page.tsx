...
const monthKey = (v: unknown) => { const d = validDate(v); return d ? `${d.getUTCFullYear()}-${String(d.getUTCMonth() + 1).padStart(2, "0")}` : null; };
...
const paidStatus = (status: unknown) => ["paid", "pago", "paid_out"].includes(String(status ?? "").toLowerCase());
const cancelledStatus = (status: unknown) => ["cancelled", "canceled", "cancelado"].includes(String(status ?? "").toLowerCase());
...
export default async function FinanceiroPage() {
  ...
  for (const p of payments ?? []) if (p.order_id && validIds.has(p.order_id)) paymentsByOrder.set(p.order_id, (paymentsByOrder.get(p.order_id) ?? 0) + n(p.amount));
  ...
  if (productIds.length) {
    const { data } = await supabase.from("product_pricing").select("product_id,labor_cost").in("product_id", productIds);
    for (const p of data ?? []) pricing.set(p.product_id, n(p.labor_cost));
  }

  const rowsMap: Record<string, Row> = {};
  let totalGross = 0, totalReceived = 0, totalCost = 0, totalFees = 0, totalLabor = 0, totalQty = 0, totalLaborItems = 0;
  for (const order of validOrders) {
    const orderItems = (items ?? []).filter((i) => i.order_id === order.id);
    const merchandise = orderItems.reduce((s, i) => s + (n(i.total) || n(i.unit_price) * n(i.quantity)), 0);
    ...
    for (const item of orderItems) {
      ...
      const qty = n(item.quantity); const base = n(item.total) || n(item.unit_price) * qty; const share = merchandise > 0 ? base / merchandise : 0;
      const row = rowsMap[category] ?? (rowsMap[category] = { category, qty: 0, gross: 0, received: 0, receivable: 0, cost: 0, fees: 0, labor: 0 });
      const laborPerUnit = item.product_id ? (pricing.get(item.product_id) ?? 0) : 0;
      row.qty += qty; row.gross += gross * share; row.received += received * share; row.cost += n(item.unit_cost) * qty; row.fees += fee * share; row.labor += laborPerUnit * qty;
      totalCost += n(item.unit_cost) * qty; totalLabor += laborPerUnit * qty; totalQty += qty; if (laborPerUnit > 0) totalLaborItems += qty;
    }
  }
  ...
  const paidExpenses = validExpenses.filter((e) => paidStatus(e.status)).reduce((s, e) => s + n(e.amount), 0);
  ...
  const recovery = totalCashOut > 0 ? Math.max(0, Math.min(100, totalReceived / totalCashOut * 100)) : 100;

  const historyMap = new Map<string, HistoryRow>();
  const ensure = (key: string) => {
    let row = historyMap.get(key);
    if (!row) { row = { key, label: monthLabel(key), gross: 0, received: 0, receivable: 0, purchases: 0, expenses: 0, equipment: 0, outflow: 0, cashResult: 0, cumulative: 0, payable: 0, payableDue: 0 }; historyMap.set(key, row); }
    return row;
  };
  for (const o of validOrders) { const k = monthKey(o.order_date); if (k) ensure(k).gross += n(o.gross_total ?? o.total); }
  for (const p of payments ?? []) { if (p.order_id && validIds.has(p.order_id)) { const k = monthKey(p.payment_date); if (k) ensure(k).received += n(p.amount); } }
  for (const p of materialPurchases ?? []) { const k = monthKey(p.created_at); if (k) ensure(k).purchases += n(p.total_cost); }
  for (const e of validExpenses) {
    const paid = paidStatus(e.status); const paymentKey = monthKey(e.paid_at); const createdKey = monthKey(e.created_at); const dueKey = monthKey(e.due_date || e.created_at);
    if (paid && paymentKey) { const row = ensure(paymentKey); if (e.source_type === "machine_purchase") row.equipment += n(e.amount); else row.expenses += n(e.amount); }
    if (!paid && dueKey) { const row = ensure(dueKey); row.payableDue += n(e.amount); }
  ...
  const now = new Date(); const currentKey = `${now.getUTCFullYear()}-${String(now.getUTCMonth() + 1).padStart(2, "0")}`;
  ...
  for (const row of historyRows) {
    ...
    const [y, m] = row.key.split("-").map(Number); const end = new Date(Date.UTC(y, m, 0, 23, 59, 59, 999));
    row.receivable = validOrders.reduce((s, o) => {
      const od = validDate(o.order_date); if (!od || od > end) return s;
      const receivedByEnd = (payments ?? []).filter(p => p.order_id === o.id && validIds.has(o.id) && validDate(p.payment_date) && validDate(p.payment_date)! <= end).reduce((a, p) => a + n(p.amount), 0);
    ...
    row.payable = validExpenses.reduce((s, e) => {
      ...
      const created = validDate(e.created_at); if (!created || created > end) return s;
      const paid = paidStatus(e.status); const paidAt = validDate(e.paid_at);
  ...
  return <div className="content">
    ...
    <div className="grid four-col" style={{ marginTop: 18 }}>
      ...
      <div className="card"><span className="muted">Caixa disponível acumulado</span><h2 className={cashBalance < 0 ? "error" : ""}>{money(cashBalance)}</h2><small className="muted">Recebido − saídas pagas</small></div>
      <div className="card"><span className="muted">A receber</span><h2>{money(Math.max(totalGross - totalReceived, 0))}</h2><small className="muted">Vendas ainda não recebidas</small></div>
      <div className="card"><span className="muted">Contas a pagar</span><h2 className={payableTotal > 0 ? "error" : ""}>{payableTotal > 0 ? money(-payableTotal) : money(0)}</h2><small className="muted">Obrigações em aberto</small></div>
    </div>

    <div className="grid four-col" style={{ marginTop: 12 }}>
      <div className="card"><span className="muted">Saídas acumuladas</span><h2>{money(totalCashOut)}</h2><small className="muted">Compras + despesas pagas + equipamentos</small></div>
      <div className="card"><span className="muted">Resultado operacional</span><h2 className={operatingResult < 0 ? "error" : ""}>{money(operatingResult)}</h2><small className="muted">Antes de considerar o caixa</small></div>
      <div className="card"><span className="muted">Recuperação do caixa</span><h2>{recovery.toLocaleString("pt-BR", { maximumFractionDigits: 1 })}%</h2><small className="muted">Recebimentos ÷ saídas acumuladas</small></div>
      <div className="card"><span className="muted">Taxas de marketplace</span><h2>{money(totalFees)}</h2><small className="muted">{validOrders.filter(o => n(o.marketplace_fee) > 0).length} pedidos com taxa</small></div>
    </div>

    <FinanceTabs summary={{ qty: totalQty, gross: totalGross, received: totalReceived, receivable: Math.max(totalGross - totalReceived, 0), profit: operatingResult }} rows={rows} fees={{ total: totalFees, count: validOrders.filter(o => n(o.marketplace_fee) > 0).length }} labor={{ total: totalLabor, items: totalLaborItems }} spent={totalCashOut} initialLaborHourRate={n(settings?.labor_hour_rate ?? 30)} />
    <FinancialHistory rows={historyRows} totalPayable={payableTotal} totalReceivable={Math.max(totalGross - totalReceived, 0)} totalOutflow={initialOutflow} totalGross={totalGross} totalReceived={totalReceived} />
