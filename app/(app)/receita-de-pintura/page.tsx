...
export default async function ReceitaPinturaPage() {
  ...
  const [{ data: recipes }, { data: templates }, { data: products }] = await Promise.all([
    supabase
      ...
      .select(
        "id,name,category,description,colors,dilution,finish,notes,created_at,products(name,product_images(public_url,sort_order))"
      )
      .order("created_at", { ascending: false }),
    supabase
      .from("painting_recipe_templates")
      .select("id,name,category,description,colors,dilution,finish,notes")
      .order("category")
      .order("name"),
    ...
    <div className="content">
      <div className="section-title">
        <div>
          ...
          <p className="muted">
            Biblioteca de misturas e processos para repetir suas pinturas com consistência.
          </p>
        </div>
      </div>
              ...
              {!recipes?.length && (
                <tr>
                  <td colSpan={5} className="muted">
                    Sua biblioteca ainda está vazia. Adicione uma receita inicial ou crie a sua.
                  </td>
                </tr>
              )}
            </tbody>
