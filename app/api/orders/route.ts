...
export async function POST(request:Request){
  ...
  try{
    ...
    const {data:products,error:pe}=await supabase.from("products").select("id,name,sale_price,estimated_cost").in("id",ids);if(pe)throw pe;
    ...
    let feePercent=0; let fixedFee=0;
    if(body.sales_channel_id){
      const {data:channel,error:ce}=await supabase.from("sales_channels").select("fee_percent,fixed_fee,active").eq("id",body.sales_channel_id).single();
      if(ce) throw ce;
      if(!channel?.active) throw new Error("O canal selecionado está inativo.");
      feePercent=Number(channel.fee_percent||0); fixedFee=Number(channel.fixed_fee||0);
    }
