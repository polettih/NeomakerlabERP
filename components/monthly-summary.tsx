...
type Month = { label: string; gross: number; received: number; expensesPaid: number; result: number };
...
function DeltaTag({ current, previous, invert }: { current: number; previous: number; invert?: boolean }) {
  ...
  const sign = d > 0 ? "+" : "";
  return <small className={good ? "kpi-green" : "error"} style={{ marginLeft: 8 }}>{sign}{d.toLocaleString("pt-BR", { maximumFractionDigits: 1 })}% vs. mês anterior</small>;
}

export function MonthlySummary({ current, previous }: { current: Month; previous: Month | null }) {
  return (
    <div className="card" style={{ marginTop: 18 }}>
      <div className="section-title"><div><h2>Resumo do mês — {current.label}</h2><p className="muted">{previous ? `Comparado a ${previous.label}.` : "Ainda não há mês anterior para comparar."}</p></div></div>
      <div className="grid four-col">
        ...
        <div className="card">
          ...
          {previous && <DeltaTag current={current.expensesPaid} previous={previous.expensesPaid} invert />}
        </div>
        <div className="card">
