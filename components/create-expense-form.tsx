...
export function CreateExpenseForm() {
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      setDescription(""); setCategory(""); setAmount(""); setDueDate(""); setPaid(false);
      r.refresh();
  ...
  return (
    <form onSubmit={submit} className="card grid">
      ...
      <div className="field" style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
        <input id="paid-now" type="checkbox" checked={paid} onChange={(e) => setPaid(e.target.checked)} />
        <label htmlFor="paid-now" style={{ marginBottom: 0 }}>Já foi paga</label>
      </div>
      <button className="btn btn-primary" disabled={busy}>{busy ? "Salvando..." : "Salvar despesa"}</button>
    </form>
  );
}
